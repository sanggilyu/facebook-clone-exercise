...
router.post("/post/new", isLoggedIn, upload.single("image"), (req, res) => {
    if (req.body.content) {  // 요청 본문에 content 필드가 존재하는지 확인합니다.
        let newPost = {};  // 새로운 게시물을 담을 빈 객체를 생성합니다.
        if (req.file) {  // 이미지 파일이 요청에 첨부되었는지 확인합니다.
            cloudinary.uploader.upload(req.file.path, result => {
                // 이미지 파일을 클라우드에 업로드하고 업로드 결과를 받아옵니다.
                newPost.image = result.secure_url;  // 업로드된 이미지의 보안 URL을 newPost 객체에 저장합니다.
                newPost.creator = req.user;  // 게시물 작성자를 newPost 객체에 저장합니다.
                newPost.time = new Date();  // 현재 시간을 newPost 객체에 저장합니다.
                newPost.likes = 0;  // 좋아요 수를 0으로 초기화하고 newPost 객체에 저장합니다.
                newPost.content = sanitize(req.body.content);  // 게시물 내용에 sanitize 함수를 적용하여 HTML 요소를 제거하고 newPost 객체에 저장합니다.
                return createPost(newPost, req, res);  // createPost 함수를 호출하여 새로운 게시물을 생성하고 응답을 반환합니다.
            });
        } else {
            newPost.image = null;  // 이미지 파일이 첨부되지 않은 경우 newPost 객체에 이미지를 null로 설정합니다.
            newPost.creator = req.user;  // 게시물 작성자를 newPost 객체에 저장합니다.
            newPost.time = new Date();  // 현재 시간을 newPost 객체에 저장합니다.
            newPost.likes = 0;  // 좋아요 수를 0으로 초기화하고 newPost 객체에 저장합니다.
            newPost.content = sanitize(req.body.content);  // 게시물 내용에 sanitize 함수를 적용하여 HTML 요소를 제거하고 newPost 객체에 저장합니다.
