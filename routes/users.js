...
const csrf = require('csurf');  // csurf 모듈을 불러옵니다.
const csrfProtection = csrf({ cookie: true });  // 쿠키를 사용하여 CSRF 보호를 설정합니다.
...
router.post(
    "/user/login",
    csrfProtection,  // CSRF 보호를 적용합니다.
    passport.authenticate("local", {  // passport를 사용하여 로컬 인증을 수행합니다.
        successRedirect: "/",  // 로그인 성공 시 리다이렉트할 경로를 지정합니다.
        failureRedirect: "/user/login"  // 로그인 실패 시 리다이렉트할 경로를 지정합니다.
    }),
    (req, res) => { }  // 요청 핸들러 함수입니다. 현재는 빈 함수로 정의되어 있습니다.
);
